...
parser MyParser(packet_in packet,                                  /*coloca o pacote recebido na var packet*/
    ...
    state start {
        transition parse_ethernet;						        /*transição por padrao nao fazemos nada aqui e esta é a 1ª transição para a qual vamos*/
    }
    /*It is common to define one state for each header type that the parser will extract, but this is not required.*/
    ...
    state parse_ethernet {                            /*extrair o header de ethernet do pacote*/
        ...
        transition select(hdr.ethernet.etherType) {		/*é um switch baseado no valor da atributo etherType no hdr.ethernet, agora que extraimos o ethernet vai ramificar consoante os seus valores*/
            TYPE_IPV4: parse_ipv4;						        /*se for TYPE_IPV4, transicionamos para parse_ipv4*/
...
control MyIngress(inout headers hdr,
    ...
    action drop() {														                          /*define a acção(funcao) drop, açoes tb podem ser globais*/
        mark_to_drop(standard_metadata);								                /*marca o pacote atual como futuro drop, assim as proximas etapas ja sabem o que fazer*/
    }

    /*!!!action parameters must specify direction, são Directional, estes n precisam porque vem da tabela do .json, são directionless */
    ...
    apply {									        /*bloco de controlo, onde sao tomadas as decisoes, é o que corre quando entramos na ingress*/
        if (hdr.ipv4.isValid()) {		/*avaliar a validade do header "hdr" aqui ainda OG*/
