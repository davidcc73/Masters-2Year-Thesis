...
control MyVerifyChecksum(inout headers hdr, inout metadata meta) {		/*é um placeholder, nao preciso para esta solucao*/
    apply {  }
}
...
control MyIngress(inout headers hdr,
    ...
    action ipv4_forward(macAddr_t dstAddr, egressSpec_t port) {			/*vamos alterar o hdr(copia do header) para depois usar como novo header do pacote, os arg vem da aplicaçao da tabela*/
            standard_metadata.egress_spec = port;						/*salvar o port de saida do pacote no sistema, para a tabela n indicar deve ser o port default*/
            hdr.ethernet.srcAddr = hdr.ethernet.dstAddr;				/*mudar MAC src no header para o dest*/
            hdr.ethernet.dstAddr = dstAddr;								/*mudar MAC dst no header para dest real*/
            hdr.ipv4.ttl = hdr.ipv4.ttl - 1;
        }

    table ipv4_lpm {						/*define uma tabela chamada ipv4_lpm*/
        key = {
            hdr.ipv4.dstAddr: lpm;			/*procurar entrada na tabela que tem como key o valor hdr.ipv4.dstAddr, pelo método LPM (Longest Prefix Match)*/
        }
        actions = {							/*lista de prioridades de açcões, caso haja mais de uma correspondencia na tabela*/
            ipv4_forward;					/*será a acção sempre aplicada em caso de correspondeica*/
            drop;							/*redundantes, mas uteis em cenários em que as condições mudam com o tempo ou com políticas mais granular*/
            NoAction;						
        }
        size = 1024;						/*max size da tabela*/
        default_action = drop();			/*acçao caso n haja entrada na tabela*/
    }
...
control MyComputeChecksum(inout headers  hdr, inout metadata meta) {	/*é um placeholder, nao preciso para esta solucao*/
     apply {
        update_checksum(
        hdr.ipv4.isValid(),
            { hdr.ipv4.version,
