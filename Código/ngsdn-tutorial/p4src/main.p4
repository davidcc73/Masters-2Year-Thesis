...
//CPU REFERES TO THE CONTROLLER OF THE NETWORK
                         
/*Headers used to carry the original switch ingress port of a packet-in, and to specify the intended output port for a packet-out*/

/*When the P4Runtime agent in Stratum receives a packet from the switch CPU port, expects to find the cpu_in_header_t header as the first. 
Indeed, it looks at the controller_packet_metadata part of the P4Info file to determine the number of bits to strip at the beginning 
of the frame and to populate the corresponding metadata field of the PacketIn message, including the ingress port as in this case.*/

/*Similarly, when Stratum receives a P4Runtime PacketOut message, it uses the values found in the PacketOut's metadata fields 
to serialize and prepend a cpu_out_header_t to the frame before feeding it to the pipeline parser*/
...
// Here we use it to carry the original ingress port where the packet was received.
@controller_header("packet_in")
header cpu_in_header_t {
    port_num_t  ingress_port;   // Port where the packet was transmitted to the controller (switch prespective)
    bit<7>      _pad;
}      
...
// PacketOut metadata fields. Here we use it to inform the P4 pipeline on which port this packet-out should be transmitted.
@controller_header("packet_out")
header cpu_out_header_t {
    port_num_t  egress_port;    // Port where the packet was transmitted to the controller (controller prespective)
    bit<7>      _pad;
}
                   ...
                   inout standard_metadata_t standard_metadata)
{
    state start {
        transition select(standard_metadata.ingress_port) {
            CPU_PORT: parse_packet_out;                         /*se o port de entrada for type CPU(para um controlador) queremos extrair inf a mais*/
...
control IngressPipeImpl (inout parsed_headers_t    hdr,
    ...
    // corresponding brinding and routing entries.
    ...
    apply {
        ...
        if (do_l3_l2) { //quando n sabemos a solucao
...
control EgressPipeImpl (inout parsed_headers_t hdr,
    ...
    apply {
        ...
        if (standard_metadata.egress_port == CPU_PORT) {       //se pacote defenido para ir para o contolador
            // *** TODO EXERCISE 4
            ...
            exit;                                           //tava nas soluçoes, mas ate faz sentido
        }
